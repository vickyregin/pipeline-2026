...
@Component({
    ...
    template: `
    <div class="max-w-7xl mx-auto animate-in fade-in duration-700 p-8 h-full overflow-y-auto space-y-8 scrollbar-hide pb-20">
      
      <!-- Horizontal Representative Selector -->
      <div class="flex gap-4 mb-8 overflow-x-auto pb-4 scrollbar-hide mask-fade-right">
        <button *ngFor="let rep of salesService.reps()" 
            (click)="selectedRepId.set(rep.id)" 
            class="group flex flex-col items-center gap-4 px-6 py-6 rounded-3xl border transition-all min-w-[160px] relative overflow-hidden"
            [ngClass]="selectedRepId() === rep.id ? 'bg-white border-blue-500 ring-8 ring-blue-500/5 shadow-2xl -translate-y-2' : 'bg-white border-slate-100 opacity-60 hover:opacity-100 hover:border-slate-300'">
            
            <div class="relative">
              <img [src]="rep.avatar" [alt]="rep.name" class="w-16 h-16 rounded-2xl object-cover border-2 transition-transform group-hover:scale-110"
                   [ngClass]="selectedRepId() === rep.id ? 'border-blue-500 shadow-lg shadow-blue-500/20' : 'border-slate-50' " />
              <div *ngIf="selectedRepId() === rep.id" class="absolute -bottom-1 -right-1 w-4 h-4 bg-blue-600 rounded-full border-2 border-white flex items-center justify-center">
                 <div class="w-1.5 h-1.5 bg-white rounded-full animate-pulse"></div>
              </div>
            </div>

            <div class="text-center">
              <div class="font-black text-sm tracking-tight" [ngClass]="selectedRepId() === rep.id ? 'text-slate-900' : 'text-slate-500'">{{ rep.name }}</div>
              <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Operational ID</div>
            </div>
      ...
      <div *ngIf="selectedRep() as rep" class="space-y-8">
        <!-- Main Header & Simulator -->
        <div class="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden relative group">
          <div class="absolute -top-32 -right-32 w-96 h-96 bg-blue-500/5 rounded-full blur-[100px] pointer-events-none"></div>
          
          <div class="p-10 border-b border-slate-50 bg-slate-50/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <span class="text-[9px] font-black text-blue-500 uppercase tracking-[0.3em] mb-1">Human Capital Intelligence</span>
              <h2 class="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-4 mt-1">
                 Performance Dashboard 
                 <span class="text-sm font-bold text-slate-400 px-4 py-1 bg-white rounded-full border border-slate-100 shadow-sm">{{ rep.name }}</span>
              </h2>
            </div>
            <div class="text-right">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-1">Current Yield (WON)</span>
                <span class="text-4xl font-black text-slate-950 tracking-tight tabular-nums">{{ metrics().totalWonRevenue | inr }}</span>
            </div>
          </div>
          ...
          <div class="p-10 grid grid-cols-1 lg:grid-cols-2 gap-16">
            <div class="space-y-10">
               ...
               <div class="bg-slate-900 rounded-[2.5rem] p-8 space-y-6 relative overflow-hidden group/attain shadow-2xl">
                  ...
                  <div class="flex justify-between items-baseline relative z-10">
                     ...
                     <span class="text-3xl font-black text-white tabular-nums">{{ metrics().achievement.toFixed(1) }}%</span>
                  </div>
                  ...
                  <div class="flex justify-between items-center text-[10px] font-black uppercase tracking-widest relative z-10">
            ...
            <div class="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-xl relative overflow-hidden group/payout flex flex-col justify-center">
              <div class="absolute -top-12 -right-12 w-48 h-48 bg-emerald-500/5 rounded-full blur-3xl"></div>
               
              <h3 class="text-[10px] font-black mb-10 flex items-center gap-3 uppercase tracking-[0.2em] text-slate-400">
              ...
              <div class="space-y-8 relative z-10">
                 <div class="flex justify-between items-end border-b border-slate-50 pb-6">
                    <span class="text-slate-400 text-[10px] font-black uppercase tracking-widest">Base Protocol (5%)</span>
                    <span class="font-black text-slate-900 text-lg tabular-nums">{{ metrics().commission.base | inr }}</span>
                 </div>
                 <div class="flex justify-between items-end border-b border-slate-50 pb-6">
                    <span class="text-slate-400 text-[10px] font-black uppercase tracking-widest">Accelerator Bonus (10%)</span>
                    <span class="font-black text-emerald-600 text-lg tabular-nums">+ {{ metrics().commission.bonus | inr }}</span>
                 </div>
                 <div class="pt-4 text-center pb-4">
                    <span class="text-slate-300 text-[9px] uppercase tracking-[0.4em] font-black block mb-4">Total Gross Payout Estimate</span>
                    <div class="text-6xl font-black bg-clip-text text-transparent bg-gradient-to-tr from-slate-900 to-slate-500 tracking-tighter tabular-nums drop-shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <!-- Business Type Split -->
          <div class="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col items-center group/dna relative overflow-hidden">
            <div class="absolute inset-0 bg-slate-50/30 opacity-0 group-hover/dna:opacity-100 transition-opacity"></div>
            <h3 class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] mb-10 w-full relative z-10">DNA Allocation (WON)</h3>
            <div class="flex-1 w-full flex flex-col justify-center gap-10 relative z-10">
              <div class="space-y-3">
                ...
                <p class="text-[9px] font-black text-slate-400 text-right uppercase tracking-widest">{{ ((metrics().newBusinessRevenue / (metrics().totalWonRevenue || 1)) * 100).toFixed(0) }}% Contribution</p>
              </div>
              ...
              <div class="space-y-3">
                ...
                <p class="text-[9px] font-black text-slate-400 text-right uppercase tracking-widest">{{ ((metrics().existingBusinessRevenue / (metrics().totalWonRevenue || 1)) * 100).toFixed(0) }}% Contribution</p>
              </div>
            </div>
          </div>

          <!-- Pipeline Funnel Metrics -->
          <div class="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-lg lg:col-span-2 relative group/funnel overflow-hidden">
            <div class="absolute -top-24 -right-24 w-64 h-64 bg-slate-50 rounded-full blur-3xl"></div>
            <h3 class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] mb-10 w-full relative z-10">Operational Funnel Load</h3>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-6 relative z-10">
              <div *ngFor="let stage of stageMetrics()" class="p-6 rounded-[2rem] border border-slate-100 bg-slate-50 group/item hover:border-slate-300 hover:bg-white transition-all transform hover:-translate-y-1">
                <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ stage.name }}</div>
                <div class="flex items-baseline gap-2 mb-2">
                  <span class="text-2xl font-black text-slate-900 tabular-nums">{{ stage.count }}</span>
                  <span class="text-[9px] font-bold text-slate-400 uppercase">Ops</span>
                </div>
                <div class="text-[11px] font-black text-slate-500 tabular-nums tracking-tighter">{{ stage.value | inr }}</div>
              </div>

              <!-- Cumulative Summary -->
              <div class="p-8 rounded-[2rem] bg-slate-950 text-white flex flex-col justify-center relative shadow-2xl overflow-hidden group/cumul">
                ...
                <div class="relative z-10">
                  ...
                  <div class="text-2xl font-black text-white tabular-nums tracking-tighter leading-none">{{ metrics().cumulativeRevenue | inr }}</div>
                  <div class="text-[8px] font-bold text-slate-500 mt-3 uppercase tracking-widest">Aggregate Pipeline & WON</div>
                </div>
              </div>
            </div>
          </div>
